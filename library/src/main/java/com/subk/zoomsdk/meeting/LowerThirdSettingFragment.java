...
import com.subk.zoomsdk.R;
import com.subk.zoomsdk.cmd.CmdLowerThirdRequest;
import com.subk.zoomsdk.cmd.LowerThirdColorType;
import com.subk.zoomsdk.meeting.rawdata.RawDataRenderer;
import com.subk.zoomsdk.meeting.util.DisplayUtil;
import com.subk.zoomsdk.meeting.util.SharePreferenceUtil;
import com.subk.zoomsdk.meeting.view.CycleColorImageView;
import com.subk.zoomsdk.meeting.view.LowerThirdLayout;
...
public class LowerThirdSettingFragment extends BottomSheetDialogFragment {
    ...
    private boolean isUseRawDataRender = false;
    ...
    private void subscribeVideoByUser(ZoomVideoSDKUser user) {
        ...
        if (!isUseRawDataRender) {
            ...
            ret = user.getVideoCanvas().subscribe(zoomCanvas, aspect);
        } else {
            user.getVideoPipe().unSubscribe(rawDataRenderer);
            ret = user.getVideoPipe().subscribe(ZoomVideoSDKVideoResolution.VideoResolution_1080P, rawDataRenderer);
        }
    ...
    private void onFragmentFinish(boolean save) {
        ...
        if (!isUseRawDataRender) {
            ...
            if (ZoomVideoSDK.getInstance().getSession() != null && rawDataRenderer != null) {
                ZoomVideoSDKUser mySelf = ZoomVideoSDK.getInstance().getSession().getMySelf();
                mySelf.getVideoPipe().unSubscribe(rawDataRenderer);
            }
        }
    }
}
